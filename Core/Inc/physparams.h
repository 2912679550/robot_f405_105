...
const float thPosPidI[2] = 
    {2.f, 1.2f}; // 从前到后为主驱动轮、辅助驱动轮的参数，暂时设置为相同
const float thPosPidD[2] = 
    {0.f, 0.f}; // 从前到后为主驱动轮、辅助驱动轮的参数，暂时设置为相同
const float thPosPidIband[2] = 
    {0.1, 0.1}; // 从前到后为主驱动轮、辅助驱动轮的参数，暂时设置为相同
// 舵轮控制中最大角速度
const float maxSteerOmg = 0.8;  //0722,降速版  PI; // 理论上最大11rad/s
const float maxScrewOmg = 20.0f; // 限制丝杠最大速度
// * 舵电机速度环
const float thVelPidP[2] = 
    {1500.f, 1000.f}; // 从前到后为主驱动轮、辅助驱动轮的参数，暂时设置为相同
const float thVelPidI[2] = 
    {4000.f, 6500.f}; // 从前到后为主驱动轮、辅助驱动轮的参数，暂时设置为相同
const float thVelPidD[2] = 
    {500.0f, 1000.0f}; // 从前到后为主驱动轮、辅助驱动轮的参数，暂时设置为相同
const float thVelPidIband[2] = 
    {1.f, 1000.f}; // 从前到后为主驱动轮、辅助驱动轮的参数，暂时设置为相同
