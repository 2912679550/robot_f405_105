...
// 驱动电机减速比
const float wheelRatio[2] = {36.0 * 19.71 , 36.0 * 40.0 / 14.0}; // 从电机转子rpm到车轮rpm       19.0f * 2.7551
const float steerRatio[2] = {36.0 * 125.0/15.0 , 36.0 * 81.0 / 15.0}; // 从电机转子rpm到舵向rpm       19.0f * 2.7551
...
const float thPosPidP[2] = 
    {10.f, 1.8f}; // 从前到后为主驱动轮、辅助驱动轮的参数，暂时设置为相同
const float thPosPidI[2] = 
    {10.f, 1.2f}; // 从前到后为主驱动轮、辅助驱动轮的参数，暂时设置为相同
...
const float thVelPidP[2] = 
    {2000.f, 3000.f}; // 从前到后为主驱动轮、辅助驱动轮的参数，暂时设置为相同
const float thVelPidI[2] = 
    {15000.f, 1000.f}; // 从前到后为主驱动轮、辅助驱动轮的参数，暂时设置为相同
const float thVelPidD[2] = 
    {5000.0f, 500.0f}; // 从前到后为主驱动轮、辅助驱动轮的参数，暂时设置为相同
const float thVelPidIband[2] = 
    {1.f, 1.f}; // 从前到后为主驱动轮、辅助驱动轮的参数，暂时设置为相同

// * 轮电机速度环
const float vVelPidP[2] = 
    {2.5f, 4000.0f}; // 从前到后为主驱动轮、辅助驱动轮的参数，暂时设置为相同
const float vVelPidI[2] = 
    {25.0f, 500.0f}; // 从前到后为主驱动轮、辅助驱动轮的参数，暂时设置为相同
const float vVelPidD[2] = 
    {3.0f, 500.0f}; // 从前到后为主驱动轮、辅助驱动轮的参数，暂时设置为相同
